...
export default async function CategoryPage({
  ...
  // 곧 최신 연도 순서와 같다 - 그룹 정렬도 그 순서를 그대로 따른다.
  const companyGroups: { corpCode: string; corpName: string; cases: typeof cases }[] = [];
  const groupIndex = new Map<string, number>();
  for (const c of cases) {
    const idx = groupIndex.get(c.corpCode);
    if (idx === undefined) {
      groupIndex.set(c.corpCode, companyGroups.length);
      companyGroups.push({ corpCode: c.corpCode, corpName: c.corpName, cases: [c] });
    } else {
      companyGroups[idx].cases.push(c);
    }
  }
